...
class Board:
    ...
    def from_file(self, filename, alphabet=BasicAlphabet, separator=','):
        with open(filename) as f:
            self.__field = [l.split(separator) for l in f.read().split('\n') if l != '']
    ...
    def validate(self):
        ...
        if not all([len(line) == len(self.__alphabet) for line in self.__field]):
            return False
    ...
    def solve(self):
        ...
        while len(positions) > 0:
            if tried > len(positions):
                raise AmbiguousError("It's not possible to solve this one or there are more than one solution")
